...
export default function RenaultMeganeEtech2026AustraliaKoContent() {
  return (
    <>
          ...
          <p style={AS.p}>
            ...
            <span style={AS.highlight}>67kWh LFP 배터리</span>다.
          </p>

          <h2 style={AS.h2}>무엇이 바뀌었나</h2>

          <p style={AS.p}>
            이번 업데이트의 핵심 변경사항은 날카로워진 새 디자인,{" "}
            <span style={AS.highlight}>WLTP 기준 500km 주행거리</span>, 기존
            160kW/300Nm 모터를 유지하는 67kWh LFP 배터리,
            15&ndash;80% 충전에 단 24분이 걸리는{" "}
            <span style={AS.highlight}>165kW DC 급속충전</span>, 그리고 2027년 상반기로
            예정된 호주 출시 일정이다.
          </p>
          ...
          <p style={AS.p}>
            ...
            수용하기 위해 차체 높이가 기존보다{" "}
            <span style={AS.highlight}>20mm 높아졌다</span>.
          </p>

          <div style={AS.callout}>
            <div style={AS.calloutTitle}>급속충전</div>
          ...
          <h2 style={AS.h2}>실내 기술 업그레이드</h2>

          <p style={AS.p}>
            5도어 프렌치 크로스오버에는 원페달 드라이빙, 히트펌프, 12.3인치 디지털
            계기판, <span style={AS.highlight}>Google Gemini</span>가 탑재된 12인치
          ...
          <h2 style={AS.h2}>한 가지 아쉬운 점</h2>

          <p style={AS.p}>
            0&ndash;100km/h 가속 시간은 기존 7.4초에서{" "}
            <span style={AS.highlight}>7.6초</span>로 소폭 느려졌다. 그러나 WLTP
          ...
          <h2 style={AS.h2}>호주 출시 일정</h2>

          <p style={AS.p}>
            신형 메간 E-Tech는 올해 하반기 유럽에서 먼저 출시되며, 호주에는{" "}
            <span style={AS.highlight}>2027년 상반기</span>에 상륙할 예정이다.
